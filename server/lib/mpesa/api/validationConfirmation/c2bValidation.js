...
const c2bValidationRouter = express.Router();
  ...
  CallbackURLModel.findOne(
    ...
    (err, remoteEndPoints) => {
      ...
      if (!remoteEndPoints)
        return mpesaFunctions.handleError(
          res,
          `Remote end points for ${req.body.BusinessShortCode} not found.`,
          GENERIC_SERVER_ERROR_CODE
        );
...
const saveTransaction = (req, res, next) => {
  const transaction = new C2BTransaction({
    validation: req.body,
...
const processRemoteValidationResp = (req, res, next) => {
